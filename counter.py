...
def word_count_dict(text_files_list, dict_common_words=enTop500, dict_my_vocab=enVocabulary, min_count=0,
                    is_for_dict=False) -> int:
    """Returns a dictionary with key being number of counts
    and value being a list of words with that key.
    :rtype : int
    :param text_files_list:
    :param dict_common_words:
    :param dict_my_vocab:
    :param min_count:
    :param is_for_dict:
...
if __name__ == '__main__':  # if program was started like a script
    ...
    user_min = 10
    choose = input('"1" - Choose all \n"2" - Choose only directory \nOther key - default values\n')
    if choose == '1':
        user_path = input('Input path to folder that contains text files (e.g."Texts" or C:/1):')
        user_trash = input('Input name of most used words vocabulary:')
        user_vocab = input('Input name of your words vocabulary:')
        user_min = input("Input number of counts. Program will print only words with count >= this")
    elif choose == '2':
        user_path = input('Input path to folder that contains text files (e.g."Texts" or C:/1):')
